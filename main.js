...
document.getElementById('btn').addEventListener('click', function () {
  ...
  var modelos = {
    'models/eye/scene.gltf': { titulo: 'LIMITED PRIVACY', descripcion: 'nuestras acciones en la red son observadas y registradas', escala: { x: 1, y: 1, z: 1 } },
    'models/teddy5/scene.gltf': { titulo: 'COMMUNITY', descripcion: 'lo sencillo que puede ser conectar con otra gente y crear nuevos vínculos', escala: { x: 0.4, y: 0.4, z: 0.4 } },
    'models/sanic/scene.gltf': { titulo: 'FUNNY', descripcion: 'los memes son capaces de unir e identificar al mismo tiempo que mejora el sentido del humor', escala: { x: 0.25, y: 0.25, z: 0.25 } },
    'models/plane/scenica.gltf': { titulo: 'GLOBALIZATION', descripcion: 'conexión instantanea que supera cualquier barrera geográfica', escala: { x: 1, y: 1, z: 1 } }
  };

  var rutasModelos = Object.keys(modelos);
  var modeloEspecifico = rutasModelos[contadorModelos % rutasModelos.length];
  var modelo = modelos[modeloEspecifico];

  var tituloElement = document.getElementById('titulo');
  tituloElement.style.display = 'block';
  tituloElement.innerText = modelos[modeloEspecifico].titulo;

  var descripcionElement = document.getElementById('descripcion');
  descripcionElement.textContent = modelo.descripcion;
